...
//   1. /images/<slug>.png  - photoreal studio shot (Gemini / Nano Banana export, 928x1152, ~4:5,
//                            warm-beige studio backdrop). Tried first for every SKU.
//   2. <ProductRender>     - studio-lit animated vector render, kept only as a graceful fallback
//                            if the photo 404s.
// The photo sits inside a fixed 4:5 frame (matching the generated assets) so every card and grid
// cell lines up edge to edge - no gaps, no overlaps, no spill, no layout shift.
//
...
export default function ProductImage({ product, size = 180, float = true, fill = false }) {
  ...
  if (usePhoto) {
    const frame = fill
      ...
      : { width: size, height: Math.round(size * (5 / 4)), maxWidth: '100%' }
    return (
